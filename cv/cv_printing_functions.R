...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$coding        <- read_gsheet(sheet_id = "coding_skills")
    cv$text_blocks   <- read_gsheet(sheet_id = "text_blocks")
    cv$contact_info  <- read_gsheet(sheet_id = "contact_info")
    cv$language      <- read_gsheet(sheet_id = "language_skills")
  } else {
    # Want to go old-school with csvs?
    cv$entries_data <- readr::read_csv(paste0(data_location, "entries.csv"), skip = 1)
    cv$coding      <- readr::read_csv(paste0(data_location, "coding_skills.csv"), skip = 1)
    cv$text_blocks  <- readr::read_csv(paste0(data_location, "text_blocks.csv"), skip = 1)
    cv$contact_info <- readr::read_csv(paste0(data_location, "contact_info.csv"), skip = 1)
    cv$language     <- readr::read_csv(paste0(data_location, "language_skills.csv"), skip = 1)
  }
...
#' @param out_of The relative maximum for coding skills. Used to set what a fully filled in skill bar is.
...
#' @param out_of The relative maximum for language skills. Used to set what a fully filled in skill bar is.
